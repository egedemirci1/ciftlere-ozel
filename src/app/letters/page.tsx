...
if (typeof document !== 'undefined') {
  document.title = 'Ahu & Yaman - Mektuplar';
}
...
export default function LettersPage() {
  ...
  const fetchLetters = async () => {
    try {
      ...
      const { mockupLetters } = await import('@/lib/mockup-data');
      setLetters(mockupLetters);
    } catch (error) {
  ...
  const handleDeleteLetter = async (letterId: string) => {
    ...
    alert('Bu mockup versiyonunda mektup silinemez. Sadece mevcut mektupları görüntüleyebilirsiniz.');
  };
  ...
  const handleCreateLetter = async (e: React.FormEvent) => {
    ...
    alert('Bu mockup versiyonunda yeni mektup eklenemez. Sadece mevcut mektupları görüntüleyebilirsiniz.');
    setIsDialogOpen(false);
  };

  const unlockLetter = (letterId: string) => {
    if (password === 'test') { // Mockup şifre
      setUnlockedLetters(prev => new Set(prev).add(letterId));
      setPassword('');
    } else {
      alert('Yanlış şifre! Şifre: test');
    }
  };
