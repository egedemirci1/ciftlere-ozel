...
export function HeartCounter({ className = '', theme = 'green' }: HeartCounterProps) {
  ...
  useEffect(() => {
    ...
    const savedCount = localStorage.getItem('ahu_yaman_login_count');
    if (savedCount) {
  ...
  const incrementCounter = () => {
    ...
    localStorage.setItem('ahu_yaman_login_count', newCount.toString());
  };
...
export function useHeartCounter() {
  const incrementLoginCount = () => {
    const currentCount = parseInt(localStorage.getItem('ahu_yaman_login_count') || '0');
    localStorage.setItem('ahu_yaman_login_count', (currentCount + 1).toString());
  };
